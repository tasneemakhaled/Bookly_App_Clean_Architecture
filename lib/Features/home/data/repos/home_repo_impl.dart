...
class HomeRepoImpl extends HomeRepo{
  ...
  @override
  Future<Either<Failure, List<BookEntity>>> fetcNewestBooks()async {
  try {
    var localBooks=homeLocalDataSource.fetcNewestBooks();
      if(localBooks.isNotEmpty){
          return right(localBooks);
        }
  var books=await  homeRemoteDataSource.fetcNewestBooks();
  return right(books);
} catch (e) {
  ...
  try {
        var localBooks=homeLocalDataSource.fetchFeaturedBooks(pageNumber: pageNumber);
        if(localBooks.isNotEmpty){
          return right(localBooks);
        }
